...
export default function HomeScreen() {
  ...
  const { data: recentActivities, isLoading: loadingAct } = useQuery({
    queryKey: ['activities', 'recent'],
    queryFn: () => activityApi.list({ club_id: activeClub?.id, limit: 5 }),
    enabled: !!activeClub,
  })

  const isRefreshing = loadingLb || loadingCh || loadingAct
  const onRefresh = useCallback(() => { refetchLb() }, [])
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        {/* Challenges section */}
        {activeClub && (
          <Section title="Retos activos" onSeeAll={() => {}}>
            {loadingCh ? (
        ...
        {/* Leaderboard section */}
        {activeClub && (
          <Section title="Ranking semanal" onSeeAll={() => {}}>
            {loadingLb ? (
              <ActivityIndicator color="#6366F1" style={{ marginVertical: 16 }} />
  ...
  return (
    <View style={styles.challengeRow}>
      ...
      <View style={styles.challengeBarBg}>
        <View style={[styles.challengeBarFill, { width: `${pct}%` as any }]} />
      </View>
    </View>
  )
}
...
const styles = StyleSheet.create({
  ...
  xpCard: {
    margin: 16, backgroundColor: 'rgba(99,102,241,0.15)',
  ...
  quickActions: { flexDirection: 'row', gap: 10, paddingHorizontal: 16, marginBottom: 8 },
  actionBtn: {
  ...
  lbAvatarSmall: {
    ...
    backgroundColor: 'rgba(99,102,241,0.4)', justifyContent: 'center', alignItems: 'center',
  },
